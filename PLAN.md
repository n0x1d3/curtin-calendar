...
| `tsconfig.json` | Add `"exclude": ["vitest.config.ts"]` — required to prevent webpack/ts-loader TS6059 error (vitest.config.ts is outside `rootDir: "src"`) |
...
| `src/utils/format/formatData.test.ts` | Tests for `convertTime` **and `getLocation`** — both public exports; `getLocation` tests required to meet the ≥80% per-file line/function threshold (without them: 50% lines, 37.5% functions) |
...
| Round | Reviewer | Verdict | Issues | Date |
|-------|----------|---------|--------|------|
| 1 | gpt-5.3-codex | NEEDS FIXES | getLocation tests not in plan scope; tsconfig exclude not in plan | 2026-02-28 |
| 2 | gpt-5.3-codex | APPROVED | — | 2026-02-28 |
